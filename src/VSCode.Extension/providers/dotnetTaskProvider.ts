...
export class DotNetTaskProvider implements vscode.TaskProvider {
    ...
    private getTask(definition: vscode.TaskDefinition): vscode.Task {
        ...
        if (ConfigurationController.isAndroid()) {
            ...
            builder.conditional('-p:AndroidAttachDebugger=true', () => !ConfigurationController.profiler)
            builder.conditional(`-p:AndroidSdbTargetPort=${ConfigurationController.getDebuggingPort()}`, () => !ConfigurationController.profiler)
            builder.conditional(`-p:AndroidSdbHostPort=${ConfigurationController.getDebuggingPort()}`, () => !ConfigurationController.profiler)
        ...
        return new vscode.Task(
            ...
            res.taskDefinitionDefaultTarget,
            res.extensionId,
