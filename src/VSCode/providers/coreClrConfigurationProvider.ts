...
export class CoreClrConfigurationProvider implements vscode.DebugConfigurationProvider {
	async resolveDebugConfiguration(folder: vscode.WorkspaceFolder | undefined,
		...
		if (ConfigurationController.targetFramework.startsWith('net6') ||
			ConfigurationController.targetFramework.startsWith('net7') ||
			ConfigurationController.targetFramework.startsWith('net8') ||
			ConfigurationController.targetFramework.startsWith('net9') ||
			ConfigurationController.targetFramework === 'net10.0-ios' ||
			ConfigurationController.targetFramework === 'net10.0-maccatalyst'
		) {
			vscode.window.showErrorMessage(res.messageMonoNotSupported, { modal: true });
			return undefined;
		}
